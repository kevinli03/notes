...
Any solution where the number of people voting for $A$ is 2 larger (exactly) than the number of people voting for $B$.

-   Why?

-   That means any person who supports $A$ does not have a profitable deviation, since player $i$ that supports A, if he isn't voting, does not want to switch to vote since that incurs a cost $-c$ when their candidate is already winning, so no profitable deviation. They also don't want to switch to +1 (see below)

-   That means any person who supports $B$ does not have a profitable deviation, since player $i$ who supports B, if he isn't voting, does not want to switch to vote since that incurs a cost $-c$ when their candidate is already losing, so no profitable deviation.

Why NOT 1 larger:

-   Well, that means any person who supports $B$ who isn't voting has a profitable deviation. They can switch to voting, and cause a tie, increasing their payoff from 0 to $1-c$.

Why NOTE more than 2 larger:

-   Well, that means a player who supports $A$ who is voting has a profitable deviation. They can switch to not voting, and get rid of the cost of voting $-c$