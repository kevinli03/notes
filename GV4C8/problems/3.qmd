...
Let us define the three parties as party $A, B, C$, who choose some position $p_A, p_B, p_C \in [-1, 1]$. Let us define $m$ as the position of the median voter in the spectrum $[-1, 1]$
...
Now, let us look at the first round. If any party wins over 50% of first place votes, they outright win. If no party does that, the party with the least first place votes loses. Since the distribution is uniformly distributed, we can just look at the distance each party wins on the number line, since that is proportional to the total number of votes.

Party $i$ that deviates will win the distance $1 - \frac{1}{2} |\epsilon|$

The other two parties will win distance $1 + |\epsilon| / 2$, however, they will split the votes between themselves as they are equidistant between all the voters here, so each party gets $\frac{1+|\epsilon|/2}{2} = \frac{1}{2} + \frac{0.5|\epsilon|}{2} =\frac{1}{2} + \frac{1}{4} |\epsilon|$

Why? See figure:

![](figures/3.2.png)

Important thing is that 50% of votes (which would outright win the first round) would only be possible if a party won more than 50% of the distance between $[-1, 1]$, which is a distance of $1$.

-   Party $i$ that deviated cannot win a majority, since they will win distance $1 - \frac{1}{2} |\epsilon|$, which cannot be greater that $1$ when $|\epsilon| < 0$. Thus, party $i$ has no chance of winning outright.

-   The other two parties win $\frac{1}{2} + \frac{1}{4} |\epsilon|$, and that can only be a majority of the first round (i.e. greater than $1$ ) if $|\epsilon| > 2$. But, since $\epsilon$ is the deviation of party $i$ in one direction, it cannot possibly exceed 1. Thus, there is no case where the other two parties outright win the election in the first round.

Then, that means one party with the lowest votes will be eliminated? Which party will be eliminated? That depends on $\epsilon$. Party $i$ will survive, if it has a higher proportion of vote than the other two parties - see the inequality: $1- \frac{1}{2} |\epsilon| > \frac{1}{2} + \frac{1}{4}|\epsilon|$

$$
1-\frac{1}{2} > \frac{1}{4}|\epsilon| + \frac{1}{2}|\epsilon|
$$
...
$$
|\epsilon| < \frac{1}{2} \times \frac{4}{3}
$$
...
-   [If $|\epsilon|$ deviation distance of party $i$ is greater than 2/3, then they lose the first round, and thus have a 0 percent chance of winning the election, thus, that is not a profitable deviation.]{.underline}

-   [When $|\epsilon|$ deviation distance is less than 2/3, party $i$ will survive to the second round, and one of the other two parties will be eliminated.]{.underline}

### Looking for Profitable Deviation: Second Round

In the second scenario, we are onto the second round - a two party battle between party $i$ at position $m+\epsilon$, and the other party at $m$
